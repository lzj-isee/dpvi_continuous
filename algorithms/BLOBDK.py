...
from algorithms.BLOBCA import BLOBCA
...
class BLOBDK(BLOB):
    ...
    def one_step_update(self, step_size = None, alpha = None, grad_fn = None, potential_fn = None, **kw):
        ...
        avg_first_variation = BLOBCA.get_avg_first_variation(self.mass, potential, kernel)
        prob_list = 1 - torch.exp( - avg_first_variation.abs() * alpha * step_size)
        self.particles = duplicate_kill_particles(prob_list, avg_first_variation > 0, self.particles, noise_amp = np.sqrt(2 * step_size), mode = 'parallel')