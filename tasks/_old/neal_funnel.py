...
class funnel(object):
    def __init__(self, opts) -> None:
        ...
        self.std = np.sqrt(3)   # the length of funnel
        self.param_slop = 3 # the width of funnel
        ...
        other_dim = torch.randn(self.ref_particle_num, self.model_dim - 1, device = self.device) * first_dim.div(self.param_slop).exp().sqrt()
    ...
    def grad_logp(self, particles, features = None, labels = None):
        ...
        result[:, 0] = -particles[:, 0] / self.std**2 - (self.model_dim - 1) / 2 / self.param_slop + \
            particles[:, 1:].pow(2).sum(1) / self.param_slop / particles[:, 0].div(self.param_slop).exp() / 2
        result[:, 1:] = -particles[:, 1:] / particles[:, 0].div(self.param_slop).exp()[:, None]
        if torch.isnan(result).sum() > 0:
            raise ValueError('Nan')
        return result   # M * dim matrix

    def potential(self, particles, features = None, labels = None):
        result = particles[:, 0].pow(2) / 2 / self.std**2 + torch.sum(particles[:, 1:].pow(2) / 2 / particles[:, 0][:, None].div(self.param_slop).exp() + particles[:, 0][:, None] / 2 / self.param_slop, dim = 1)
        # result = particles[:, 0].pow(2) / 2 / self.std**2 + particles[:, 1:].pow(2).sum(1) / 2 / particles[:, 0].div(self.param_slop).exp() + \
        #     (self.model_dim - 1) * particles[:, 0] / 2 / self.param_slop
