...
class multi_gaussian(object):
    def __init__(self, opts, **kw) -> None:
        ...
        self.std = 1.0
        self.init_mu, self.init_std = 0.0, 1.0 # init distribution of particles
        ...
        # generate reference
        self.weight = torch.Tensor([2/3, 1/3]).to(self.device) # weight of mixture gaussian
        index = torch.randint(0, 3, (self.ref_particle_num, ), device = self.device)
        self.skew = 1.2
        self.ref_particles = torch.randn(self.ref_particle_num, self.model_dim, device = self.device) * self.std
        self.ref_particles = torch.cat([self.ref_particles[index < 2] + self.skew, self.ref_particles[index == 2] - self.skew], dim = 0)
    ...
    def grad_logp(self, particles, features = None, labels = None):
        part1 = self.weight[0] * torch.exp(-(particles - self.skew).pow(2).sum(1) / 2 / self.std**2)
        part2 = self.weight[1] * torch.exp(-(particles + self.skew).pow(2).sum(1) / 2 / self.std**2)
        result = - (particles - self.skew) / self.std**2 / ((part1 + part2) / part1)[:, None] - \
            (particles + self.skew) / self.std**2 / ((part1 + part2) / part2)[:, None]
    ...
    def plot_result(self, particles, mass, device):
        # create mesh
        x, y = torch.linspace(-4, 4, 81, device = device), torch.linspace(-4, 4, 81, device = device)
