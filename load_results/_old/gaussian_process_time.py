...
if __name__ == "__main__":    
    ...
    parser.add_argument('--sparse', type = float, default = 0.3)
    parser.add_argument('--x_max', type = int, default = 60)
    ...
    for name in data.keys():
        ...
        index[name] = np.linspace(0, point_num[name] - 1, int(point_num[name] * opts.sparse), dtype = int)
        ...
        x_axis[name] = np.linspace(0, opts.x_max, len(mean_w2[name]))
    # with open('./figures/gp.md', mode = 'w') as f:
    #     for name in plot_settings_common['order']:
    #         if name not in mean_w2.keys(): continue
    #         f.write('{}: \t'.format(plot_settings_common['label'][name]) + '{:.3e} & {:.3e}'.format(mean_w2[name][-1], mean_ksd[name][-1]) + '\n')
    # plot w2
    ...
    plt.savefig('./figures/gp_w2_time.{}'.format(opts.suffix), dpi = opts.dpi, bbox_inches = 'tight')
    plt.close()
    # plot ksd
    plt.figure(figsize=(7.68, 4.8))
    for name in plot_settings_common['order']:
        if name not in data.keys(): continue
        plt.plot(x_axis[name], mean_ksd[name], 
            color = plot_settings_common['color'][name],
            linestyle = plot_settings_common['linestyle'][name],
            label = plot_settings_common['label'][name],
            alpha = 1.0, 
    ...
    if opts.suffix == 'eps':
        ax = plt.gca()
        ax.set_rasterized(True)
    plt.savefig('./figures/gp_ksd_time.{}'.format(opts.suffix), dpi = opts.dpi, bbox_inches = 'tight')
    plt.close()