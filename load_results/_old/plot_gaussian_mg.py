...
def pdf_calc(particles):  # M * dim matrix
    var = 3
    cov = 0.8 * 3
    weight = torch.Tensor([2/3, 1/3]) # weight of mixture gaussian
    g_means = torch.Tensor([[1.2,1.2],[-1.2,-1.2]]) # \mu of gaussian mixture
    cov_0 = torch.Tensor([[var, -cov], [-cov, var]])
    cov_1 = torch.Tensor([[var, -cov], [-cov, var]])
    g_covs = torch.cat([cov_0.unsqueeze(0), cov_1.unsqueeze(0)], dim = 0)
    inv_0, inv_1 = torch.inverse(cov_0), torch.inverse(cov_1)
    g_invs = torch.cat([inv_0.unsqueeze(0), inv_1.unsqueeze(0)], dim = 0)
    result = weight[0] * torch.exp(-(torch.matmul(particles - g_means[0], g_invs[0]) * (particles - g_means[0])).sum(1) / 2) + \
        weight[1] * torch.exp(-(torch.matmul(particles - g_means[1], g_invs[1]) * (particles - g_means[1])).sum(1) / 2)
    return result   # M array

if __name__ == "__main__":    
    parser = argparse.ArgumentParser()
    parser.add_argument('--main_folder', type = str, default = 'results_save/gaussian_figure/result_mg')
    ...
    for folder_name in sequence:
        save_folder_this = opts.save_folder + 'mg' + folder_name[folder_name.find('_'):]
        ...
        for algorithm in data[folder_name]:
            ...
            plt.contour(grid_X.cpu().numpy(), grid_Y.cpu().numpy(), pdf.cpu().numpy(), alpha = 0.8)
            # set particle size
