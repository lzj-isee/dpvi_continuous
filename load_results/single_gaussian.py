...
if __name__ == "__main__":    
    ...
    parser.add_argument('--save_folder', type = str, default = './figures/single_gaussian')
    parser.add_argument('--dpi', type = int, default = 300)
    # for plotting particles
    parser.add_argument('--task', type = str, default = 'single_gaussian')
    ...
    scale = {'num32':(1.2, 2.4), 'num64':(1.2, 2.2), 'num128':(1.0, 1.8), 'num256':(1.0, 1.6), 'num512':(1.0, 1.6)}
    f =  open('./load_results/plot_settings.yaml', 'r')
    plot_settings = yaml.load(f.read(), Loader=yaml.FullLoader)
    # ========================================== iter ==================================================================================
    # loading_results_iteration
    main_folder = os.path.join(opts.main_folder, 'results_sg_iter')
    data_iter = {}
    ...
    with open(os.path.join(opts.save_folder, 'single_gaussian_iter.txt'), mode = 'w') as f:
    ...
    for num in particle_num:
        ...
        plt.savefig(os.path.join(opts.save_folder, 'sg_%s_iter.pdf'%(num)), dpi = opts.dpi, bbox_inches = 'tight')
        plt.close()
    # ========================================== time ==================================================================================
    # loading_results_time
    main_folder = os.path.join(opts.main_folder, 'results_sg_time')
    data_time = {}
    ...
    if not os.path.exists(opts.save_folder): os.makedirs(opts.save_folder)
    with open(os.path.join(opts.save_folder, 'single_gaussian_time.txt'), mode = 'w') as f:
        for name in plot_settings['order']:
    ...
    for num in particle_num:
        ...
        plt.ylabel('2-Wasserstein Distance', {'size': 12})
        plt.tick_params(labelsize = 12)
        plt.savefig(os.path.join(opts.save_folder, 'sg_%s_time.pdf'%(num)), dpi = opts.dpi, bbox_inches = 'tight')
        plt.close()
    # ============================================= particle ===================================================================
    # plot particles
    ...
    results_folder = os.listdir(os.path.join(opts.main_folder, 'results_sg_iter'))
    results_folder.sort()
    data = {}
    for k  in particle_num: data[k] = {}
    for num in data.keys():
        results_folder = os.listdir(os.path.join(opts.main_folder, 'results_sg_iter', num))
        results_folder.sort()
        for folder in results_folder:
            if 'S[0]' not in folder: continue
            method_name = folder[0: folder.find('_')]
            particles = np.load(os.path.join(opts.main_folder, 'results_sg_iter', num, folder, 'particles.npy'))
            mass = np.load(os.path.join(opts.main_folder, 'results_sg_iter', num, folder, 'mass.npy'))
    ...
    num = len(loc)
    log_pdf = - task.potential_2d(loc.view(-1, 2)).view(num, num)
    for num in particle_num:
    ...
    plt.savefig(os.path.join(opts.save_folder, 'sg_4in1.pdf'), dpi = opts.dpi, bbox_inches = 'tight')
    plt.close()
